# 🚀 Control System Simulator

A web-based Control System Simulator built using Python and Streamlit to analyze linear time-invariant (LTI) systems using transfer functions.python based platform to omplement the graphs and plot ,visualisation ,bode plot

---

## 🌐 Live Demo

👉 https://csanalyser-vlvxfbgjqbehadwotdjcc5.streamlit.app/

---

## 📌 Features

* 🔹 Transfer Function Input (Numerator & Denominator)
* 🔹 Step Response Plot
* 🔹 Impulse Response Plot
* 🔹 Bode Plot (Magnitude & Phase)
* 🔹 Root Locus Plot
* 🔹 Stability Analysis (Stable / Unstable)
* 🔹 Poles & Zeros Display
* 🔹 Performance Metrics:

  * Rise Time
  * Settling Time
  * Overshoot
* 🔹 Gain Margin & Phase Margin
* 🔹 Predefined Systems (First-order, Second-order, Underdamped)
* 🔹 Download Plots as Images
* 🔹 Clean and Interactive UI

---

## 🧠 Concepts Used

* Transfer Functions
* Time Domain Analysis
* Frequency Domain Analysis
* System Stability
* Root Locus Technique

---

## 🛠️ Tech Stack

* Python
* Streamlit
* NumPy
* Matplotlib
* Control Systems Library

---

## 📂 Project Structure

```
control-system-analyzer/
│
├── app.py
├── requirements.txt
├── README.md
```

---

## ⚙️ Installation (Run Locally)

```bash
git clone https://github.com/your-username/control-system-analyzer.git
cd control-system-analyzer
pip install -r requirements.txt
streamlit run app.py
```

---

## 📊 Example Transfer Function

G(s) = 1 / (s² + 2s + 1)

---

## 📷 Screenshots

*Add screenshots of your app here (recommended)*
<img src="Screenshot (56).png" width="800"/>
<img src="Screenshot (57).png" width="800"/>
<img src="Screenshot (58).png" width="800"/>
<img src="Screenshot (59).png" width="800"/>
<img src="Screenshot (60).png" width="800"/>



---

## 🚀 Deployment

This project is deployed using Streamlit Cloud.

---

## 💡 Future Improvements

* Compare multiple systems
* Add time delay systems
* Export analysis report (PDF)
* Add controller design (PID tuning)

---

## 👨‍💻 Author

**Jayashankar**
B.Tech ECE Student

---

## ⭐ Show Your Support

If you like this project, give it a ⭐ on GitHub!
